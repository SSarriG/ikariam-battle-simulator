...
import { UnitUpgradeService } from '../../src/domain/UnitUpgradeService';
import { HephaestusService } from '../../src/domain/HephaestusService';
import { UnitType, BattleType } from '../../src/domain/enums';

describe('Unit Module', () => {
    describe('Unit Creation', () => {
        test('should create a Hoplite correctly', () => {
            const hoplite = UnitFactory.createUnit('hoplita', 'unit-1');
            expect(hoplite.name).toBe('hoplita');
            expect(hoplite.type).toBe(UnitType.FirstLine);
            expect(hoplite.battleType).toBe(BattleType.Terrestrial);
        ...
        test('should create a Steam Giant correctly', () => {
            const giant = UnitFactory.createUnit('gigante-vapor', 'unit-2');
    ...
    describe('Upgrades', () => {
        test('should apply upgrade level 1 correctly', () => {
            const hoplite = UnitFactory.createUnit('hoplita', 'unit-1');
            UnitUpgradeService.applyUpgrade(hoplite, 1);
        ...
        test('should apply upgrade level 3 correctly', () => {
            const hoplite = UnitFactory.createUnit('hoplita', 'unit-1');
            UnitUpgradeService.applyUpgrade(hoplite, 3);
            const stats = hoplite.getEffectiveStats();
    ...
    describe('Hephaestus Bonus', () => {
        test('should apply Hephaestus level 5 correctly', () => {
            const giant = UnitFactory.createUnit('gigante-vapor', 'unit-1');
        ...
        test('should combine Upgrades and Hephaestus correctly', () => {
            ...
            UnitUpgradeService.applyUpgrade(giant, 3);
            // Hephaestus 5: +20 Armor, +15% Damage
            giant.hephaestusLevel = 5;
    ...
    describe('Battle Mechanics', () => {
        ...
        test('should consume ammunition', () => {
            const archer = UnitFactory.createUnit('arquero', 'unit-1');
